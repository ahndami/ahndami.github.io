...
- 문법 훑어보기
  ...
  - ECMAScript의 문법은 C 언어, 자바나 펄 등 C와 비슷한 언어에서 차용한 것이다.
    - 나의 생각: 다양한 개발 언어를 공부한다는 가능성을 염두에 두고, 설명해주고 있다.
- 대소문자 구분
...
- 식별자
  ...
  - 카멜 케이스로 쓴다. (ECMAScript의 내장 함수와 객체가 카멜 케이스로 표기되어 있어서 이 형식을 따르길 권한다.)
    - 나의 생각: 다른 책에서는 '첫 번째 문자는 숫자로 시작하면 안된다.' 정도로 알려줬었는데 이 책에서는 다양한 경우가 있다는 것을 알려주고 싶어 한다.
- 주석
  - C 언어 스타일로 표기한다.
  - 한 줄 주석
    ```
    ...
    - var 연산자는 변수를 로컬 스코프에서 정의한다.
      ...
      - 학원에서 배운 예제
        ```
          ...
          alert(message);
        ```
      - 나의 생각: 학원에서 배운 예제를 사용하면 지역, 전역 변수를 구분하지 않아도 전부 에러가 나지 않는다. 이렇게 사용했을 때의 문제점이 있을까요?
